            ...
            <div className="p-4 space-y-6">
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Add a special effect that appears around your avatar.</CardDescription>
                    </CardHeader>
                    ...
                    {avatarEffects.map(effect => {
                            ...
                            return (
                                <div 
                                    ...
                                    className={cn(
                                        "relative flex flex-col items-center justify-center gap-2 p-4 rounded-lg border-2 cursor-pointer transition-all h-36",
                                        avatarEffect === effect.id ? "border-primary bg-primary/10" : "border-transparent bg-muted/50 hover:bg-accent"
                                    )}
                                >
                                    <div className={cn("relative", isBounce && "w-16 h-16 flex items-center justify-center")}>
                                        {isBounce ? (
                                            <BounceEffect>
                                                <Avatar className="size-16"><AvatarImage src={user.photoURL || undefined} /><AvatarFallback>{user.displayName?.[0]}</AvatarFallback></Avatar>
                                            </BounceEffect>
                                        ) : (
                                            <Avatar className="size-16"><AvatarImage src={user.photoURL || undefined} /><AvatarFallback>{user.displayName?.[0]}</AvatarFallback></Avatar>
                                        )}
                                    ...
                                    <span className="font-medium text-xs mt-1">{effect.name}</span>
                                </div>
                            )
                    })}
                    </CardContent>
                ...
                <Card>
                    ...
                    {profileEffects.map(effect => (
                            <div
                                ...
                                className={cn(
                                    "relative flex flex-col items-center justify-between p-2 rounded-lg border-2 cursor-pointer transition-all flex-row gap-4",
                                    profileEffect === effect.id ? "border-primary bg-primary/10" : "border-transparent bg-muted/50 hover:bg-accent"
                                )}
                            >
                                <div className="w-full h-20 bg-accent rounded-lg relative overflow-hidden flex flex-col items-center justify-end p-1.5">
                                    <effect.component />
                                    <div className="relative z-10 w-full bg-background/50 backdrop-blur-sm p-1 rounded-md border">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <div><p className="font-bold text-sm">{user.displayName}</p></div>
                                        </div>
                                    </div>
                                </div>
                                <span className="font-medium text-sm w-24 text-center">{effect.name}</span>
                            </div>
                    ))}
                    </CardContent>
                </Card>
                 ...
                 <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Make your name stand out with a custom background.</CardDescription>
                    </CardHeader>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            {nameplates.map(plate => (
                                <div
                                    ...
                                    className={cn(
                                        "relative flex flex-col items-center justify-center gap-2 p-2 rounded-lg border-2 cursor-pointer transition-all",
                                        nameplateUrl === plate.url ? "border-primary bg-primary/10" : "border-transparent bg-muted/50 hover:bg-accent"
                                    )}
                                >
                                    <div className="w-full h-14 bg-accent rounded-lg relative overflow-hidden flex flex-col items-center justify-center">
                                        {plate.url ? (
                                            <Image src={plate.url} alt={plate.name} fill className="object-cover" data-ai-hint={plate.hint} />
                                        ): (
                                            <div className="w-full h-full bg-muted flex items-center justify-center text-muted-foreground text-xs">Default</div>
                                        )}
                                        <div className="relative z-10 w-full bg-black/30 backdrop-blur-sm p-1 rounded-md mx-2">
                                            <div className="flex items-center gap-1.5">
                                                <Avatar className="size-6"><AvatarImage src={user.photoURL || undefined} /><AvatarFallback>{user.displayName?.[0]}</AvatarFallback></Avatar>
                                                <p className="font-bold text-white text-shadow-md text-sm">{user.displayName}</p>
                                            </div>
                                        </div>
                                    </div>
                                    <span className="font-medium text-xs">{plate.name}</span>
                                </div>
                            ))}
                        </div>
                        <div className="pt-2">
